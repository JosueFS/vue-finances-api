...
function records(_, { month, type, accountsIds, categoriesIds }, ctx, info) {
  ...
  //Required filter Header -> Authorization: userId by token
  let AND = [{ user: { id: userId } }];

  //Optional filter: Type -> DEBIT | CREDIT
  AND = !type ? AND : [...AND, { type }];

  //Optional filter: Account ID List
  AND =
    !accountsIds || accountsIds.length === 0
      ? AND
      : [...AND, { OR: accountsIds.map((id) => ({ account: { id } })) }];

  //Optional filter: Category ID List
  AND =
...
function totalBalance(_, { date }, ctx, info) {
  const userId = getUserId(ctx);
  const dateISO = moment(date, 'DD-MM-YYYY')
    .endOf('day')
    .toISOString();
  ...
  return ctx.prisma.$graphql(mutation, variables).then((response) => {
    const { totalbalance: totalBalance } = response.executeRaw[0];
